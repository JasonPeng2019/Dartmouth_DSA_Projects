...
public class Polyline implements Shape {
	...
	public Shape deepCopy() {
		Polyline deepCopy = new Polyline(this.path.get(0).x, this.path.get(0).y, new Color(this.color.getRGB()));
		for (int i = 1; i < path.size(); i++){
			deepCopy.path.add(new Point(this.path.get(i).x, this.path.get(i).y));
		}
		return deepCopy;
	}
