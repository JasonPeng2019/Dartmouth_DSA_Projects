...
public class AdjacencyMapGraph<V,E> implements Graph<V,E> {
	...
	public int outDegree(V v) {
		Map<V, E> outMap = out.get(v);
		return (outMap != null) ? outMap.size() : 0;
	}

	public int inDegree(V v) {
		Map<V, E> inMap = in.get(v);
	 ...
	 * @return
	 */
	public AdjacencyMapGraph<V, E> deepCopy() {
		AdjacencyMapGraph<V, E> newGraph = new AdjacencyMapGraph<>();
		...
		// Copy edges
		for (V from : this.vertices()) {
			for (V to : this.outNeighbors(from)) {
				newGraph.insertDirected(from, to, this.getLabel(from, to));
			}
		}
